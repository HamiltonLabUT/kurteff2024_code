...
//
    ...
    var currpid:String = "S0000" // Subject ID
    var currset:Bool = true // first 25 or last 25 sentences, set in settings.
    ...
    var logFile = "S0000_B1"
    var filename:String = "mocha_50_kids"
    ...
    var totalTrials = 25 // the total number of trials in each block (echolalia/shuffled), so this *2 = total sentences read by participant
    var totalRepeats = 0 // how many combination echolalia/shuffled blocks we are doing beyond the first, default is 3 (for 400 trials)
    ...
    var path:String = "str" // stupid placeholder
    var fileStart = 0
    var fileEnd = 25
...
//            print("flist count \(fileList.count)")
    ...
    // updates our current text file then displays it in a text box
        ...
        if currentFile == fileEnd { // are we at the end of the file?
            print("This is happening")
            currentFile = fileStart //+ (ecogBlockSize*(ecogBlock-1))
        }
            ...
            currentFile += 1
            currentTrial += 1
            print("Current trial (func getAndDisplayTextFile, just added 1 to it): \(currentTrial)")
            currentRecording += 1
        }
    }
    ...
    // also called when the second (shuffled) block starts
        ...
        if let path = Bundle.main.path(forResource: filename, ofType: "txt") { // this if statement is false?
...
//            let path = Bundle.main.path(forResource: filename, ofType: "txt")!
        ...
        print(numTrials)
        currentTrial = fileStart
        print("Current trial (func loadMOCHA): \(currentTrial)")
        currentFile = fileStart //+ (ecogBlockSize*(ecogBlock-1))
        if ecogBlock == 0 {
            fileListShuffled = fileList.shuffled()
            print("This is not ecog.")
        }
        else {
            fileList = Array(fileList[((ecogBlock-1)*ecogBlockSize)..<(ecogBlock*ecogBlockSize)])
            // Slice the file list according to first half/second half indices depending on value of bool "currset" (from settings view controller)
            fileListShuffled = fileList.shuffled()
            print(fileList)
        ...
        for elem in fileListShuffled {
            ...
            if currset == true { // first 25
                if randomIndex < 25 {
                    print("Less than or equal to 25: \(randomIndex)")
                    randomIndexList.append(randomIndex)
                }
            }
            if currset == false { // second 25
                if randomIndex >= 25 {
                    print("greater than 25: \(randomIndex)")
                    randomIndexList.append(randomIndex)
                }
            }
//            randomIndexList.append(randomIndex)
...
//        if currset == true {
//            print("first 25 random")
//            randomIndexList = [Int](self.randomIndexList[0...24])
//            print(randomIndexList)
//            print("This should be 25: \(randomIndexList.count)")
//        }
//        if currset == false {
//            print("second 25 random")
//            randomIndexList = [Int](self.randomIndexList[25...49])
//            print(randomIndexList)
//            print("This should be 25: \(randomIndexList.count)")
//        }
        print(randomIndexList)
        print("randomIndexList count, should be 25: \(randomIndexList.count)")
    }
    ...
    // plays back the previous trial using information from the var currentTrial
        ...
        if block == "shuffled" {
            ...
            print("Current trial (func PlayBackTrial): \(currentTrial)")
            print("randomIndexList.count: \(randomIndexList.count)")
            print("The full random index list:")
            print(randomIndexList)
            if currset == true {
                randomTrial = randomIndexList[currentTrial-1]//-1 or no?
            }
            if currset == false {
                randomTrial = randomIndexList[currentTrial-26]//-1 or no?
            }
        ...
        if currentTrial == fileEnd  { // if we've done 10/100/X echolalia trials, move to the second block.
...
//
    ...
    // this button is pressed at the end of a break
        ...
        currentTrial = fileStart // do this instead of loading mocha again
        print("Current trial (func breakOver): \(currentTrial)")
        currentFile = fileStart //+ (ecogBlockSize*(ecogBlock-1)) // do this instead of loading mocha again
    ...
    // has a lot of functions depending on where we are in the task (described below)
        ...
        if audioRecorder == nil {
            if nextButtonPressed == true {
                ...
                // "Next" button
                if currentTrial == fileEnd && echolaliaComplete == false && currentRepeat == totalRepeats {
                // end of task
...
//            print("Hi liberty")
//            filename = String("mocha_50_kids_part1")
            totalTrials = 25
            fileStart = 0
            fileEnd = 25
            currentRecording = fileStart
            
        }
        if currset == false {
            print("Will use second 25 sentences from MOCHA 4 kids.")
//            print("Hi liberty (part 2)")
//            filename = String("mocha50_kids_part2")
            totalTrials = 25
            fileStart = 25
            fileEnd = 50
            currentRecording = fileStart
